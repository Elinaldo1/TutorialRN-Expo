...
import { BarChart, Grid } from 'react-native-svg-charts';
 ...
 export function ChartColuna(chartData, chartDimensions) {
   return chartDimensions ? (
   <BarChart
      data = {chartData.data}
      svg = {{fill: 'rgb(175, 220, 129, 0.7)'}}
      style = {{
   ...
   />
   ): <ActivityIndicator size= "large" color="#ff6" />
...
export function CustomBarChart(chartData, chartDimensions) {
        ...
        return chartDimensions ? (
         ...
         ): <ActivityIndicator size = "large" color= "#ff3"/>
       }
